...
## v3.6.3 — Stable Release
**Tag:** `v3.6.3`

### What's new
- Comment system fixed: `//`, `#`, and `/* */` (inline and multi-line) all now work correctly
- Full regression test suite added: `test_new_syntax.nat` (38 sections covering all v3.4–v3.6 features) and `test_legacy_syntax.nat` (16 sections covering v3.0 backward-compatible syntax)
- All v3.0 legacy syntax confirmed working: `num()`, `int()`, `string()`, `show(x)`, `add x with y to z`, `fix X val` (no dot), old-style `repeat i from A to B step N`
- All v3.4–v3.6 syntax confirmed working: file I/O, module system, runtime injection, graphs, clamp, split, type checks

### Bug fixes
- `/* */` block comments (inline and multi-line) were not handled at all — fixed
- Windows CRLF issue from v3.6.1 reconfirmed fixed

> **This is the first fully stable, regression-tested release. Recommended for all users.**
